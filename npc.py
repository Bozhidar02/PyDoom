...
class LostSoul(NPC):
    ...
    def movement(self):
        next_pos = self.game.pathfinder.get_path(self.map_position, self.game.player.map_position)
        next_x, next_y = next_pos
        if next_pos not in self.game.object_manager.npc_positions and next_pos not in self.map_position:
            angle = math.atan2(next_y + 1 - self.y, next_x + 1 - self.x)
            dx = math.cos(angle) * self.speed
            dy = math.sin(angle) * self.speed
            self.collision_check(dx, dy)
        print('moving')
